...
main_keyboard = ReplyKeyboardMarkup([
    ...
    [KeyboardButton("Удалить задачу"), KeyboardButton("Присоединиться к команде")],
    [KeyboardButton("main")],
...
def get_or_create_team_key(user_id):
    ...
    if result:
        ...
        cursor.execute("INSERT INTO users (user_id, team_key) VALUES (?, ?)", (user_id, team_key))
        conn.commit()
...
async def show_tasks(update: Update, context: CallbackContext) -> None:
    ...
    # Добавляем инлайн-кнопку для копирования ключа
    keyboard = [[InlineKeyboardButton("Скопировать ключ", callback_data=f"copy_{team_key}")]]
...
async def copy_team_key(update: Update, context: CallbackContext) -> None:
    query = update.callback_query
    await query.answer()
    team_key = query.data.split("_")[1]
    await query.message.reply_text(f"Командный ключ:\n```\n{team_key}\n```\nСкопируйте этот текст и поделитесь с командой!", parse_mode='Markdown')
...
async def about_creator(update: Update, context: CallbackContext) -> None:
    message = (
        "О создателе:\n"
        "Привет! Я разработчик этого бота. Создал его, чтобы помочь людям организовывать свои задачи.\n\n"
        "Если хотите поддержать развитие проекта, можете отправить TON-коины:\n"
        "Адрес кошелька TON: UQ... (вставьте свой адрес)\n\n"
...
# Начало присоединения к команде
async def join_team(update: Update, context: CallbackContext) -> int:
    context.user_data['user_id'] = update.effective_user.id
    await update.message.reply_text("Введите командный ключ участника:")
    return JOIN_TEAM

# Обработка ввода командного ключа
async def process_team_key(update: Update, context: CallbackContext) -> int:
    ...
    if result:
        cursor.execute("UPDATE users SET team_key = ? WHERE user_id = ?", (team_key, user_id))
        conn.commit()
        conn.close()
        await update.message.reply_text(f"Вы успешно присоединились к команде с ключом {team_key}!")
        await show_tasks(update, context)
        return ConversationHandler.END
    else:
        conn.close()
...
async def handle_message(update: Update, context: CallbackContext) -> None:
    ...
    elif text == "Присоединиться к команде":
...
conv_handler = ConversationHandler(
    entry_points=[
        ...
        MessageHandler(filters.Regex("^(Присоединиться к команде)$"), join_team),
    ],
    states={
        ...
        JOIN_TEAM: [MessageHandler(filters.TEXT & ~filters.COMMAND, process_team_key)],
    },
    fallbacks=[
...
def main() -> None:
    ...
    application.add_handler(CallbackQueryHandler(copy_team_key, pattern="^copy_"))
