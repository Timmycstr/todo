...
# НОВОЕ: Клавиатура сортировки
sort_keyboard = InlineKeyboardMarkup([
    [InlineKeyboardButton("По дате", callback_data="sort_deadline")],
    [InlineKeyboardButton("По приоритету", callback_data="sort_priority")]
])
...
def init_db():
    ...
    # Добавляем колонку status в таблицу tasks
    cursor.execute("""
...
async def show_tasks(update: Update, context: CallbackContext) -> None:
    ...
    # НОВОЕ: Последние 5 задач
    cursor.execute("SELECT id, description, status FROM tasks ORDER BY id DESC LIMIT 5")
    recent_tasks = cursor.fetchall()
    
    conn.close()

    message = f"Список задач (командный ключ: `{team_key}`):\n"
    
    # Форматируем основные задачи
    if not rows:
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("Закрыть задачу", callback_data="close")]
    ]
...
async def handle_sort(update: Update, context: CallbackContext):
    ...
    if query.data == "sort_deadline":
        ...
        context.user_data['sort_order'] = "CASE priority WHEN 'высокий' THEN 1 WHEN 'средний' THEN 2 WHEN 'низкий' THEN 3 END"
    
    await show_tasks(update, context)


# НОВОЕ: Закрытие задачи
async def close_task(update: Update, context: CallbackContext):
    ...
    await query.message.reply_text("Задача закрыта!")
    await show_tasks(update, context)

# НОВОЕ: Редактирование задач
...
# НОВОЕ: Обработчик выбора поля для редактирования
async def edit_field(update: Update, context: CallbackContext):
    ...
    if field == "status":
        keyboard = [
            ...
            [InlineKeyboardButton("Просрочено", callback_data="status_просрочено")]
        ]
        await query.message.edit_text(
        ...
        await query.message.edit_text(f"Введите новое значение для поля '{field}':")
    
    return edit_task_menu

# НОВОЕ: Сохранение изменений
        ...
        try:
            ...
            for threshold in notify_thresholds:
                if time_left <= threshold:
                    time_str = f"{time_left.seconds // 3600}ч {(time_left.seconds // 60) % 60}м" if time_left > timedelta(0) else "Просрочено"
            ...
            logging.error(f"Ошибка проверки дедлайна для задачи {task_id}: {e}")
...
conv_handler = ConversationHandler(
    ...
    states={
        ...
        edit_task_menu: [CallbackQueryHandler(edit_task_menu)],  # НОВОЕ
        edit_field: [MessageHandler(filters.TEXT & ~filters.COMMAND, save_edit)],  # НОВОЕ
    },
    fallbacks=[
